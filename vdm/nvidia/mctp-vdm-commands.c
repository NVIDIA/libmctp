...
/*
 * Debug token query v3:
 * Query debug token status with version 3 protocol
 */
int debug_token_query_v3(int fd, uint8_t tid, uint8_t verbose)
{
	uint8_t *resp = NULL;
	size_t resp_len = 0;
	mctp_requester_rc_t rc = -1;
	...
	/* Encode the VDM headers for debug token query*/
	mctp_encode_vendor_cmd_dbg_token_query_v3(&cmd);
