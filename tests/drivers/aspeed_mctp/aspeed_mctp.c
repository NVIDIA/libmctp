 ...
 * 
 */
static ssize_t mock_mctp_mtd_read(struct file *file, char __user *data,
				  size_t size, loff_t *offset)
{
	...
	if (missed != 0) {
		printk(KERN_INFO
		       "[mock_mctp_mtd_read] Copy to user missed %d bytes\n",
		       missed);
	}
 ...
 * 
 */
static ssize_t mock_mctp_mtd_write(struct file *filp, const char *buff,
				   size_t len, loff_t *off)
{
	printk(KERN_INFO "[mock_mctp_mtd_write] proc file write (len = %lu)\t",
	       len);
	...
	if (missed != 0) {
		...
		printk(KERN_ERR
		       "[mock_mctp_mtd_write] Error in coping data from user (%lu vs %lu)\n",
		       len, packet->len);
	}
...
static struct proc_ops mock_mctp_proc_ops = { .proc_read = mock_mctp_mtd_read,
					      .proc_write =
						      mock_mctp_mtd_write };
 ...
 * 
 */
static int mock_mctp_driver_cdev_in_release(struct inode *inode,
					    struct file *file)
{
 ...
 * 
 */
static long mock_mctp_driver_cdev_in_ioctl(struct file *file, unsigned int cmd,
					   unsigned long arg)
{
 ...
 */
static unsigned int
mock_mctp_driver_cdev_in_poll(struct file *filp, struct poll_table_struct *wait)
{
	__poll_t mask = 0;
 ...
 * 
 */
static ssize_t mock_mctp_driver_cdev_in_read(struct file *file,
					     char __user *data, size_t size,
					     loff_t *offset)
{
	printk(KERN_INFO
	       "[mock_mctp_driver_cdev_in_read] Read device (size = %lu)\n",
	       size);
	...
	if ((mock_mctp_current_packet_rin != NULL) &&
		...
		if (ret != 1) {
			printk(KERN_ERR
			       "[mock_mctp_driver_cdev_in_read] Error in getting packet from fifo\n");
			return -EAGAIN;
		} else {
	...
	size_t bytes_in_packet = mock_mctp_current_packet_rin->len -
				 mock_mctp_current_packet_rin->offset;
	...
	if (missed != 0) {
		...
		printk(KERN_ERR
		       "[mock_mctp_driver_cdev_in_read] Error in coping data to user, missed = %lu\n",
		       missed);
	}
 ...
 * 
 */
static ssize_t mock_mctp_driver_cdev_in_write(struct file *file,
					      const char __user *data,
					      size_t size, loff_t *offset)
{
	printk(KERN_INFO
	       "[mock_mctp_driver_cdev_in_write] Write device (size = %lu)\n",
	       size);
	...
	if (missed != 0) {
		...
		printk(KERN_ERR
		       "[mock_mctp_driver_cdev_in_write] Error in coping data from user (%lu vs %lu)\n",
		       size, packet->len);
	}
	...
	if (skip_packets_in == 1) {
		printk(KERN_INFO
		       "[mock_mctp_driver_cdev_in_write] Skipping received packet, size = %lu\n",
		       packet->len);
		kfree(packet->buf);
		kfree(packet);
	} else {
 ...
 * 
 */
static int mock_mctp_driver_cdev_out_open(struct inode *inode,
					  struct file *file)
{
 ...
 * 
 */
static int mock_mctp_driver_cdev_out_release(struct inode *inode,
					     struct file *file)
{
 ...
 * 
 */
static long mock_mctp_driver_cdev_out_ioctl(struct file *file, unsigned int cmd,
					    unsigned long arg)
{
 ...
 */
static unsigned int
mock_mctp_driver_cdev_out_poll(struct file *filp,
 ...
 * 
 */
static ssize_t mock_mctp_driver_cdev_out_read(struct file *file,
					      char __user *data, size_t size,
					      loff_t *offset)
{
	printk(KERN_INFO
	       "[mock_mctp_driver_cdev_out_read] Read device (size = %lu)\n",
	       size);
	...
	if ((mock_mctp_current_packet_rout != NULL) &&
		...
		if (ret != 1) {
			printk(KERN_ERR
			       "[mock_mctp_driver_cdev_out_read] Error in getting packet from fifo\n");
			return -EAGAIN;
		} else {
	...
	size_t bytes_in_packet = mock_mctp_current_packet_rout->len -
				 mock_mctp_current_packet_rout->offset;
	size_t bytes_to_read = (bytes_in_packet <= size) ? bytes_in_packet :
							   size;
	...
	if (missed != 0) {
		...
		printk(KERN_ERR
		       "[mock_mctp_driver_cdev_out_read] Error in coping data to user, missed = %lu\n",
		       missed);
	}
 ...
 * 
 */
static ssize_t mock_mctp_driver_cdev_out_write(struct file *file,
					       const char __user *data,
					       size_t size, loff_t *offset)
{
	printk(KERN_INFO
	       "[mock_mctp_driver_cdev_out_write] Write device (size = %lu)\n",
	       size);
	...
	if (missed != 0) {
		_mock_mctp_counters.errors_copy_from_user_out += 1;
		printk(KERN_ERR
		       "[mock_mctp_driver_cdev_out_write] Error in coping data from user (%lu vs %lu)\n",
		       size, packet->len);
	}
	kfifo_put(&mock_mctp_fifo_out, packet);
 ...
 * 
 */
static int mock_mctp_dev_uevent(const struct device *dev,
...
static int __init mock_mctp_module_init(void)
{
	...
	drv->mctp_entry = proc_create(mock_mctp_proc_name, 0666, NULL,
				      &mock_mctp_proc_ops);
	...
	if (drv->mctp_entry != NULL) {
		...
		printk(KERN_ERR
		       "[mock_mctp_module_init] Failed to created /proc/%s!\n",
		       mock_mctp_proc_name);
		result = -1;
		goto on_error;
	}
	...
	if (result >= 0) {
		printk(KERN_INFO
		       "[mock_mctp_module_init] Succeed alloc chrdev region as major number %d!\n",
		       result);
	} else {
		printk(KERN_ERR
		       "[mock_mctp_module_init] Could not alloc chrdev region!\n");
		result = -ENOMEM;
		goto on_error;
	}
	...
	if (drv->kernel_cdev_in == NULL) {
		printk(KERN_ERR
		       "[mock_mctp_module_init] Failed to alloc cdev\n");
		result = -ENOMEM;
		goto on_error;
	}
	...
	if (drv->kernel_class_in == NULL) {
		printk(KERN_ERR
		       "[mock_mctp_module_init] Failed to create kernel class\n");
		result = -1;
		goto on_error;
	}
	...
	if (IS_ERR(device_create(drv->kernel_class_in, NULL, drv->dev_in, NULL,
				 "%s", (const char *)mock_mctp_dev_name))) {
		printk(KERN_ERR
		       "[mock_mctp_module_init] Failed to create device\n");
		result = -1;
		goto on_error;
	}
	...
	drv->kernel_class_out =
		class_create(THIS_MODULE, mock_mctp_dev_name_out);
#endif
	...
	if (IS_ERR(device_create(drv->kernel_class_out, NULL, drv->dev_out,
				 NULL, "%s",
				 (const char *)mock_mctp_dev_name_out))) {
		printk(KERN_ERR
		       "[mock_mctp_module_init] Failed to create device\n");
		result = -1;
		goto on_error;
