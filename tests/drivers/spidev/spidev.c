 ...
 * 
 */
static ssize_t mock_mctp_spi_mtd_read(struct file *file, char __user *data, size_t size, loff_t *offset) {
    int rv;
	int missed;
	char *buf_ptr;
    ...
    if(missed != 0) {
        printk(KERN_INFO "[mock_mctp_spi_mtd_read] Copy to user missed %d bytes\n", missed);
    }
 ...
 * 
 */
static ssize_t mock_mctp_spi_mtd_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
	int missed;
	struct data_pkt *packet;

    printk(KERN_INFO "[mock_mctp_spi_mtd_write] proc file write (len = %lu)\t", len);
	...
	if (missed != 0) {
		...
		printk(KERN_ERR "[mock_mctp_spi_mtd_write] Error in coping data from user (%lu vs %lu)\n", len, packet->len);
	}
 ...
 * 
 */
static int mock_mctp_spi_driver_cdev_in_open(struct inode *inode, struct file *file) {
 ...
 * 
 */
static int mock_mctp_spi_driver_cdev_in_release(struct inode *inode, struct file *file) {
	printk(KERN_INFO "[mock_mctp_spi_driver_cdev_in_release] Release device\n");
	return 0;
}
...
long mock_mctp_spidev_message(struct spi_ioc_transfer *ioc, unsigned n_ioc)
{
	...
	if (n_ioc == 1) {
		if ((ioc->len >= 3) && (ioc->len < 256)) {
			missed =  copy_from_user(tx_buf, (const u8 __user *) (uintptr_t) ioc->tx_buf, ioc->len);
			if ((tx_buf[0] == CMD_SREG_W32) && (tx_buf[1] == 0) && (tx_buf[2] == SPI_SPIM2EC_MBX)) {
				if (tx_buf[3] == AP_REQUEST_RESET) {
					printk(KERN_DEBUG "[mock_mctp_spidev_message] Get [SPIM2ECMB] AP_REQUEST_RESET\n");
					gpiod_mock_gen_irq(SPB_GPIO_INTR_NUM);
				} 
				else if ((tx_buf[0] == CMD_SREG_W32) && (tx_buf[1] == 0) && (tx_buf[2] == SPI_SPIM2EC_MBX) && (tx_buf[3] == AP_REQUEST_WRITE)) {
					printk(KERN_DEBUG "[mock_mctp_spidev_message] Get [SPIM2ECMB] AP_REQUEST_WRITE\n");
					gpiod_mock_gen_irq(SPB_GPIO_INTR_NUM);
				} 
				else if ((tx_buf[0] == CMD_SREG_W32) && (tx_buf[1] == 0) && (tx_buf[2] == SPI_SPIM2EC_MBX) && (tx_buf[3] == AP_READY_TO_READ)) {
					printk(KERN_DEBUG "[mock_mctp_spidev_message] Get [SPIM2ECMB] AP_READY_TO_READ\n");
					gpiod_mock_gen_irq(SPB_GPIO_INTR_NUM);
				} 
				else if ((tx_buf[0] == CMD_SREG_W32) && (tx_buf[1] == 0) && (tx_buf[2] == SPI_SPIM2EC_MBX) && (tx_buf[3] == AP_FINISHED_READ)) {
					printk(KERN_DEBUG "[mock_mctp_spidev_message] Get [SPIM2ECMB] AP_FINISHED_READ\n");
					gpiod_mock_gen_irq(SPB_GPIO_INTR_NUM);
				}
				else {
					printk(KERN_DEBUG "[mock_mctp_spidev_message] Get length = %d\n", tx_buf[6]);
					gpiod_mock_gen_irq(SPB_GPIO_INTR_NUM);
				}
			} 
			else if ((tx_buf[0] == CMD_SREG_R32) && (tx_buf[1] == 0) && (tx_buf[2] == SPI_EC2SPIM_MBX)) {
				// Ack request
				printk(KERN_DEBUG "[mock_mctp_spidev_message] Get ACK request command\n");
				tx_buf[3] = EC_ACK;
				missed = copy_to_user((void *) (uintptr_t) ioc->rx_buf, tx_buf, 4);
			}
			else if ((tx_buf[0] == CMD_POLL_ALL) && (tx_buf[1] == 0)) {
				printk(KERN_DEBUG "[mock_mctp_spidev_message] Get CMD_POLL_ALL command\n");
				tx_buf[0] = 0x2f;
				tx_buf[1] = 0;
				tx_buf[2] = 0;
				tx_buf[3] = 0x01;
				tx_buf[4] = 0x08;
				missed = copy_to_user((void *) (uintptr_t) ioc->rx_buf, tx_buf, 5);
			}
			else {
				printk(KERN_DEBUG "[mock_mctp_spidev_message] Get request: len=%d: %02x %02x %02x %02x, missed = %d\n", 
					ioc->len, tx_buf[0], tx_buf[1], tx_buf[2], tx_buf[3], missed);
			}
		} 
		else {
			printk(KERN_WARNING "[mock_mctp_spidev_message] Expected at least 3 bytes message, but is only %d\n", ioc->len);
		}
		...
		printk(KERN_WARNING "[mock_mctp_spidev_message] Received %u messages - unsupported for now\n", n_ioc);
	}
 ...
 * 
 */
static long mock_mctp_spi_driver_cdev_in_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	unsigned		n_ioc;
	long 			retval;
	...
	case SPI_IOC_RD_MODE:
		...
		break;
	default:
		printk(KERN_INFO "[mock_mctp_spi_driver_cdev_in_ioctl] Ioctl device - cmd = 0x%08x, 0xarg = %08lx\n", cmd, arg);
		return 1;
		break;
	}
 ...
 * 
 */
static unsigned int mock_mctp_spi_driver_cdev_in_poll(struct file *filp, struct poll_table_struct *wait)
{
	__poll_t mask = 0;
 ...
 * 
 */
static ssize_t mock_mctp_spi_driver_cdev_in_read(struct file *file, char __user *data, size_t size, loff_t *offset) {
	size_t copied;
	size_t missed;
	size_t bytes_to_read;
	size_t bytes_in_packet;

	printk(KERN_INFO "[mock_mctp_spi_driver_cdev_in_read] Read device (size = %lu)\n", size);
	...
	if ((mock_mctp_current_packet_rin != NULL) && (mock_mctp_current_packet_rin->offset > 0)) {
		...
		if (ret != 1) {
			printk(KERN_ERR "[mock_mctp_spi_driver_cdev_in_read] Error in getting packet from fifo\n");
			return -EAGAIN;
		}
		else {
	...
	bytes_in_packet = mock_mctp_current_packet_rin->len - mock_mctp_current_packet_rin->offset;
	bytes_to_read = (bytes_in_packet <= size) ? bytes_in_packet : size;

	missed = copy_to_user(data, &(mock_mctp_current_packet_rin->buf[mock_mctp_current_packet_rin->offset]), bytes_to_read);
	if (missed != 0) {
		_mock_mctp_counters.errors_copy_to_user_in += 1;
		printk(KERN_ERR "[mock_mctp_spi_driver_cdev_in_read] Error in coping data to user, missed = %lu\n", missed);
	}
 ...
 * 
 */
static ssize_t mock_mctp_spi_driver_cdev_in_write(struct file *file, const char __user *data, size_t size, loff_t *offset) {
	int missed;
	struct data_pkt *packet;

	printk(KERN_INFO "[mock_mctp_spi_driver_cdev_in_write] Write device (size = %lu)\n", size);
	...
	if (missed != 0) {
		...
		printk(KERN_ERR "[mock_mctp_spi_driver_cdev_in_write] Error in coping data from user (%lu vs %lu)\n", size, packet->len);
	}
	...
	if (skip_packets_in == 1) {
		printk(KERN_INFO "[mock_mctp_spi_driver_cdev_in_write] Skipping received packet, size = %lu\n", packet->len);
		kfree(packet->buf );
		kfree(packet);
	}
	else {
 ...
 * 
 */
static int mock_mctp_spi_driver_cdev_out_open(struct inode *inode, struct file *file) {
 ...
 * 
 */
static int mock_mctp_spi_driver_cdev_out_release(struct inode *inode, struct file *file) {
	printk(KERN_INFO "[mock_mctp_spi_driver_cdev_release] Release device\n");
	return 0;
}
 ...
 * 
 */
static long mock_mctp_spi_driver_cdev_out_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	printk(KERN_INFO "[mock_mctp_spi_driver_cdev_in_ioctl] Ioctl device - cmd = %u, arg = %lu\n", cmd, arg);
 ...
 * 
 */
static unsigned int mock_mctp_spi_driver_cdev_out_poll(struct file *filp, struct poll_table_struct *wait)
{
	__poll_t mask = 0;
 ...
 * 
 */
static ssize_t mock_mctp_spi_driver_cdev_out_read(struct file *file, char __user *data, size_t size, loff_t *offset) {
	size_t copied;
	size_t missed;
	size_t bytes_to_read;
	size_t bytes_in_packet;

	printk(KERN_INFO "[mock_mctp_spi_driver_cdev_out_read] Read device (size = %lu)\n", size);
	...
	if ((mock_mctp_current_packet_rout != NULL) && (mock_mctp_current_packet_rout->offset > 0)) {
		...
		if (ret != 1) {
			printk(KERN_ERR "[mock_mctp_spi_driver_cdev_out_read] Error in getting packet from fifo\n");
			return -EAGAIN;
		}
		else {
	...
	bytes_in_packet = mock_mctp_current_packet_rout->len - mock_mctp_current_packet_rout->offset;
	bytes_to_read = (bytes_in_packet <= size) ? bytes_in_packet : size;

	missed = copy_to_user(data, &(mock_mctp_current_packet_rout->buf[mock_mctp_current_packet_rout->offset]), bytes_to_read);
	if (missed != 0) {
		_mock_mctp_counters.errors_copy_to_user_out += 1;
		printk(KERN_ERR "[mock_mctp_spi_driver_cdev_out_read] Error in coping data to user, missed = %lu\n", missed);
	}
 ...
 * 
 */
static ssize_t mock_mctp_spi_driver_cdev_out_write(struct file *file, const char __user *data, size_t size, loff_t *offset) {
	int missed;
	struct data_pkt *packet;

	printk(KERN_INFO "[mock_mctp_spi_driver_cdev_out_write] Write device (size = %lu)\n", size);
	...
	if (missed != 0) {
		_mock_mctp_counters.errors_copy_from_user_out += 1;
		printk(KERN_ERR "[mock_mctp_spi_driver_cdev_out_write] Error in coping data from user (%lu vs %lu)\n", size, packet->len);
	}
	kfifo_put(&mock_mctp_fifo_out, packet);
 ...
 * 
 */
static int mock_mctp_spi_dev_uevent(const struct device *dev, struct kobj_uevent_env *env)
{
...
static int __init mock_mctp_spi_module_init(void) {
	...
	drv->mctp_entry = proc_create(mock_mctp_spi_proc_name, 0666, NULL, &mock_mctp_spi_proc_ops);

    if (drv->mctp_entry != NULL) {
        printk(KERN_INFO "[mock_mctp_spi_module_init] /proc/%s created\n", mock_mctp_spi_proc_name);
    }
    else {
        printk(KERN_ERR "[mock_mctp_spi_module_init] Failed to created /proc/%s!\n", mock_mctp_spi_proc_name);
        result = -1;
		goto on_error;
    }

	// Alloc MAJOR number
	result = alloc_chrdev_region(&(drv->dev_in), 0, 1, mock_mctp_spi_dev_name);
	if (result >= 0) {
		printk(KERN_INFO "[mock_mctp_spi_module_init] Succeed alloc chrdev region as major number %d!\n", result);
	}
	else {
		printk(KERN_ERR "[mock_mctp_spi_module_init] Could not alloc chrdev region!\n");
		result = -ENOMEM;
		goto on_error;
	}
	...
	if (drv->kernel_cdev_in == NULL) {
		printk(KERN_ERR "[mock_mctp_spi_module_init] Failed to alloc cdev\n");
		result = -ENOMEM;
		goto on_error;
	}
	...
	if (result < 0) {
		printk(KERN_ERR "[mock_mctp_spi_module_init] Failed to add cdev\n");
		goto on_error;
	}
...
#else
	drv->kernel_class_in = class_create(THIS_MODULE, mock_mctp_spi_dev_name);
#endif
	if (drv->kernel_class_in == NULL) {
		printk(KERN_ERR "[mock_mctp_spi_module_init] Failed to create kernel class\n");
		result = -1;
		goto on_error;
	}
	...
	// Create device
	if (IS_ERR(device_create(drv->kernel_class_in, NULL, drv->dev_in, NULL, "%s", (const char*)mock_mctp_spi_dev_name))) {
		printk(KERN_ERR "[mock_mctp_spi_module_init] Failed to create device\n");
		result = -1;
		goto on_error;
	}

	// Alloc MAJOR number
	result = alloc_chrdev_region(&(drv->dev_out), 0, 1, mock_mctp_spi_dev_name_out);
	if (result >= 0) {
		printk(KERN_INFO "[mock_mctp_spi_module_init] Succeed alloc chrdev region as major number %d!\n", result);
	}
	else {
...
#else
	drv->kernel_class_out = class_create(THIS_MODULE, mock_mctp_spi_dev_name_out);
#endif
	...
	// Create device
	if (IS_ERR(device_create(drv->kernel_class_out, NULL, drv->dev_out, NULL, "%s", (const char*)mock_mctp_spi_dev_name_out))) {
		printk(KERN_ERR "[mock_mctp_spi_module_init] Failed to create device\n");
		result = -1;
		goto on_error;
	}
