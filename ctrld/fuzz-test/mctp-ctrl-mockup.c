...
#define LOG_FR(format, ...) do { \
        if (g_fuzz_verbose_level >= 2) { \
            fprintf(stdout, "[FR at %s:%d]: ", __func__, __LINE__); \
            LOG(format, ##__VA_ARGS__); \
        } \
    } while (0)

#define LOG_ML(format, ...) do { \
        if (g_fuzz_verbose_level >= 2) { \
            fprintf(stdout, "[ML at %s:%d]: ", __func__, __LINE__); \
            LOG(format, ##__VA_ARGS__); \
        } \
    } while (0)
...
#define LOG_ERR(format, ...) do { \
    if (g_fuzz_verbose_level >= 1) {\
        fprintf(stdout, "[FR][ERR at %s:%d]: ", __func__, __LINE__); \
...
static int manage_rx_packets(int client_fd, int bytes_received)
{
    ...
    if (bytes_received < (rec_msg_length + 4)) {
        ...
        */
        LOG_FR("Expect messages of at least %d bytes, ignoring", rec_msg_length + 4);
        return 0;
    }
    ...
    if ((mctp_req_buf[rec_msg_length] == 0) && (mctp_req_buf[rec_msg_length +1] == 0x80) 
        && (mctp_req_buf[rec_msg_length +2] == MCTP_CTRL_CMD_PREPARE_ENDPOINT_DISCOVERY)) {
        struct mctp_ctrl_resp_prepare_discovery  prep_discovery_resp;
        encode_ctrl_cmd_header(&hdr, getRqDgramInst(), MCTP_CTRL_CMD_PREPARE_ENDPOINT_DISCOVERY);
        ...
        memcpy(response, &prep_discovery_resp, sizeof(prep_discovery_resp));
    } 
    /* MCTP_CTRL_CMD_ENDPOINT_DISCOVERY */
    else if ((mctp_req_buf[rec_msg_length] == 0) && (mctp_req_buf[rec_msg_length +1] == 0x80) 
        && (mctp_req_buf[rec_msg_length +2] == MCTP_CTRL_CMD_ENDPOINT_DISCOVERY)) {
        struct mctp_ctrl_resp_endpoint_discovery resp_ep_discovery;
        encode_ctrl_cmd_header(&hdr, getRqDgramInst(), MCTP_CTRL_CMD_ENDPOINT_DISCOVERY);
    ...
    /* MCTP_SET_EP_REQUEST */
        ...
        && (mctp_req_buf[rec_msg_length +2] == MCTP_CTRL_CMD_SET_ENDPOINT_ID)) {
        struct mctp_ctrl_resp_set_eid resp_set_eid;
        encode_ctrl_cmd_header(&hdr, getRqDgramInst(), MCTP_CTRL_CMD_SET_ENDPOINT_ID);
    ...
    /* MCTP_ALLOCATE_EP_ID_REQUEST */
        ...
        && (mctp_req_buf[rec_msg_length +2] == MCTP_CTRL_CMD_ALLOCATE_ENDPOINT_IDS)) {
        struct mctp_ctrl_resp_alloc_eid resp_alloc_eid;
        encode_ctrl_cmd_header(&hdr, getRqDgramInst(), MCTP_CTRL_CMD_ALLOCATE_ENDPOINT_IDS);
    ...
    /* MCTP_GET_ROUTING_TABLE_ENTRIES_REQUEST */
        ...
        && (mctp_req_buf[rec_msg_length +2] == MCTP_CTRL_CMD_GET_ROUTING_TABLE_ENTRIES)) {
        struct mctp_ctrl_resp_get_routing_table get_routing_table;
        struct get_routing_table_entry routing_table_entry;
        int entries_cnt=0;
        
        encode_ctrl_cmd_header(&hdr, getRqDgramInst(), MCTP_CTRL_CMD_GET_ROUTING_TABLE_ENTRIES);
        ...
        for(int i=0; i<entries_cnt; ++i) {
            ...
            routing_table_entry.phys_transport_binding_id = getUint8t();
            ...
            if (resp_len > (sizeof(struct mctp_ctrl_resp) - sizeof(struct get_routing_table_entry)))
                break;
        }
    }
    /* MCTP_CTRL_CMD_GET_ENDPOINT_UUID */
    else if ((mctp_req_buf[rec_msg_length] == 0) && (mctp_req_buf[rec_msg_length +1] == 0x80) 
        && (mctp_req_buf[rec_msg_length +2] == MCTP_CTRL_CMD_GET_ENDPOINT_UUID)) {
        struct mctp_ctrl_resp_get_uuid resp_get_uuid;
        encode_ctrl_cmd_header(&hdr, getRqDgramInst(), MCTP_CTRL_CMD_GET_ENDPOINT_UUID);
    ...
    /* MCTP_CTRL_CMD_GET_MESSAGE_TYPE_SUPPORT */
        ...
        && (mctp_req_buf[rec_msg_length +2] == MCTP_CTRL_CMD_GET_MESSAGE_TYPE_SUPPORT)) {
        struct mctp_ctrl_resp_get_msg_type_support resp_get_msg_type_support;
        int entries_cnt=0;

        encode_ctrl_cmd_header(&hdr, getRqDgramInst(), MCTP_CTRL_CMD_GET_MESSAGE_TYPE_SUPPORT);
        resp_get_msg_type_support.completion_code = MCTP_CTRL_CC_SUCCESS;
        resp_get_msg_type_support.msg_type_count = entries_cnt = getUint8t();

        resp_len = sizeof(resp_get_msg_type_support);
        memcpy(response, &resp_get_msg_type_support, sizeof(resp_get_msg_type_support));

        for(int i=0; i<entries_cnt; ++i) {
            ((uint8_t*)response)[resp_len++] = getUint8t();
            if (resp_len > (sizeof(struct mctp_ctrl_resp) - sizeof(struct mctp_ctrl_resp_get_msg_type_support)))
                break;
        }
    }
    /* MCTP_CTRL_CMD_GET_ENDPOINT_ID */
    else if ((mctp_req_buf[rec_msg_length] == 0) && (mctp_req_buf[rec_msg_length +1] == 0x80) 
        && (mctp_req_buf[rec_msg_length +2] == MCTP_CTRL_CMD_GET_ENDPOINT_ID)) {
        struct mctp_ctrl_resp_get_eid resp_get_eid;

        encode_ctrl_cmd_header(&hdr, getRqDgramInst(), MCTP_CTRL_CMD_GET_ENDPOINT_ID);
    ...
    /* MCTP_CTRL_CMD_GET_VERSION_SUPPORT */
        ...
        && (mctp_req_buf[rec_msg_length +2] == MCTP_CTRL_CMD_GET_VERSION_SUPPORT)) {
        struct mctp_ctrl_resp_get_mctp_ver_support resp_get_mctp_ver_support;
        int entries_cnt=0;

        encode_ctrl_cmd_header(&hdr, getRqDgramInst(), MCTP_CTRL_CMD_GET_VERSION_SUPPORT);
        resp_get_mctp_ver_support.completion_code = MCTP_CTRL_CC_SUCCESS;
        resp_get_mctp_ver_support.number_of_entries = entries_cnt = getUint8t();

        resp_len = sizeof(resp_get_mctp_ver_support);
        memcpy(response, &resp_get_mctp_ver_support, sizeof(resp_get_mctp_ver_support));
        ...
        for(int i=0; i<entries_cnt; ++i) {
            ...
            if (resp_len > (sizeof(struct mctp_ctrl_resp) - sizeof(struct mctp_ctrl_resp_get_mctp_ver_support)))
                break;
        }
    }
    ...
    if (resp_len > 0) {
        ...
        LOG_FR("Response length = %zi, prefix length = %zi", resp_len, mctp_prefix_len);
    
        flipRandomBits(((uint8_t*)mctp_resp_buf), resp_len);
        LOG_HEX((uint8_t*)mctp_resp_buf, resp_len, "write");

        int send_return_code;
        if ((send_return_code = write(client_fd, mctp_resp_buf, resp_len)) == -1) {
            LOG_ERR("Write error %d, response is not sent, errno = %d", send_return_code, errno);
//               err(EXIT_FAILURE, "Write error %d", send_return_code);
        ...
        LOG_FR("  0x%02x, 0x%02x, 0x%02x", mctp_req_buf[rec_msg_length], 
            mctp_req_buf[rec_msg_length +1],mctp_req_buf[rec_msg_length +2]);
    }
...
static int manage_rx_vendor_packets(int client_fd, int bytes_received)
{
    ...
    if (mctp_req_buf[0] != 0x0A) {
        LOG_ERR("Expected tag header byte of VDM: 0x0A, but received 0x%02x", mctp_req_buf[0]);
        return 0;
    }
    if (mctp_req_buf[2] != MCTP_MESSAGE_TYPE_VDIANA) {
        LOG_ERR("Expected message type VDIANA 0x7F, but received 0x%02x", mctp_req_buf[2]);
        return 0;
    }
    if (mctp_req_buf[3] != 0) {
        LOG_ERR("Expected iana value 0x00, but received 0x%02x", mctp_req_buf[3]);
        return 0;
    }
    if (mctp_req_buf[4] != 0) {
        LOG_ERR("Expected iana value 0x00, but received 0x%02x", mctp_req_buf[4]);
        return 0;
    }
    if (mctp_req_buf[5] != 0x16) {
        LOG_ERR("Expected iana value 0x16, but received 0x%02x", mctp_req_buf[5]);
        return 0;
    }
    if (mctp_req_buf[6] != 0x47) {
        LOG_ERR("Expected iana value 0x47, but received 0x%02x", mctp_req_buf[6]);
        return 0;
    }
    ...
    if ((mctp_req_buf[8] == 0x01) && (mctp_req_buf[9] == MCTP_VENDOR_CMD_BOOTCOMPLETE) 
    ...
    /* MCTP_VENDOR_CMD_ENABLE_HEARTBEAT */
    else if ((mctp_req_buf[8] == 0x01) && (mctp_req_buf[9] == MCTP_VENDOR_CMD_ENABLE_HEARTBEAT) 
        ...
        mctp_resp_buf[7] = 0x80; /* rq_dgram_inst */
        mctp_resp_buf[8] = 0x01; /* vendor_msg_type */
        mctp_resp_buf[9] = MCTP_VENDOR_CMD_ENABLE_HEARTBEAT; /* command_code */
        mctp_resp_buf[10] = 0x01; /* msg_version */
        resp_len = 11;
    }
    /* MCTP_VENDOR_CMD_HEARTBEAT */
    else if ((mctp_req_buf[8] == 0x01) && (mctp_req_buf[9] == MCTP_VENDOR_CMD_HEARTBEAT) 
        && (mctp_req_buf[10] == 0x01)) {
        mctp_resp_buf[2] = MCTP_VENDOR_MSG_TYPE;
        /* struct mctp_vendor_msg_hdr */
        mctp_resp_buf[3] = 0; /* iana */
...
void* runFuzzerReceiver(void* arg)
{
	...
	if (g_socket_path[0] == '\0') {
        ...
        LOG_FR("Open socket with name: %s, len = %d",
            g_socket_path, namelen);
	}
    ...
    while (msg_cnt < msg_end) {
            ...
            if (client_fd < 0) {
                ...
                if ((client_fd = accept(server_fd, (struct sockaddr *)&client_addr, &client_len)) < 0) {
                    if (errno != EWOULDBLOCK && errno != EAGAIN) {
                        ...
                        LOG_FR("Accept for the UUT finished with errno = %d", errno);
                    }
                }
                else {
                    // Connection accepted, handle it
                    LOG_FR("Incoming connection from the UUT accepted, socket = %d", client_fd);
                ...
                if ((client_hb_fd = accept(server_fd, (struct sockaddr *)&client_addr, &client_len)) < 0) {
                    if (errno != EWOULDBLOCK && errno != EAGAIN) {
                        LOG_ERR("accept failed");
                        close(server_fd);
                        return NULL;
                    ...
                    // Connection accepted, handle it
                    LOG_FR("Incoming connection from the UUT accepted, socket = %d", client_hb_fd);
                    setsockopt(client_hb_fd, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));
        ...
        if (fds[1].revents & POLLIN) {
            int bytes_received = recv(client_fd, mctp_req_buf, MCTP_MAX_CTRL_MESSAGE_SIZE, 0);
            if (bytes_received < 0) {
                if (errno != EAGAIN) {
                    LOG_FR("error receiving data, errno=%d", errno);
                    err(EXIT_FAILURE, "error receiving data");
                } else {
                    LOG_FR("recv timeout");
                ...
                LOG_FR("recv ok [1], msg length: %d", bytes_received);
                LOG_HEX(mctp_req_buf, bytes_received, "read");

                if ((bytes_received == 1) && (client_type < 0)) {
                    if (mctp_req_buf[0] == MCTP_MESSAGE_TYPE_MCTP_CTRL) {
                        LOG_FR("Client type MCTP CTRL");
                        client_type = MCTP_MESSAGE_TYPE_MCTP_CTRL;
                    }
                    else if (mctp_req_buf[0] == MCTP_MESSAGE_TYPE_VDIANA) {
                        LOG_FR("Client type VDIANA");
                        client_type = MCTP_MESSAGE_TYPE_VDIANA;
                    }
                }
                else if (client_type == MCTP_MESSAGE_TYPE_MCTP_CTRL) {
                    if (manage_rx_packets(client_fd, bytes_received) < 0) {
                        break;
                    }
                ...
                LOG_FR("recv - 0 length response, errno=%d", errno);
                zero_length_response_cnt++;
                if ((msg_cnt > 1) || (zero_length_response_cnt > 3)) {
                    break;
                }
                else {
                    continue;
        ...
        if (fds[2].revents & POLLIN) {
            int bytes_received = recv(client_hb_fd, mctp_req_buf, MCTP_MAX_CTRL_MESSAGE_SIZE, 0);
            if (bytes_received < 0) {
                if (errno != EAGAIN) {
                    LOG_FR("error receiving data, errno=%d", errno);
                    err(EXIT_FAILURE, "error receiving data");
                ...
                LOG_FR("recv ok [2], msg length: %d", bytes_received);
                LOG_HEX(mctp_req_buf, bytes_received, "read");

                if ((bytes_received == 1) && (client_hb_type < 0)) {
                    if (mctp_req_buf[0] == MCTP_MESSAGE_TYPE_MCTP_CTRL) {
                        LOG_FR("SPI heart beat thread is a client type MCTP CTRL");
                        client_hb_type = MCTP_MESSAGE_TYPE_MCTP_CTRL;
                    }
                    else if (mctp_req_buf[0] == MCTP_MESSAGE_TYPE_VDIANA) {
                        LOG_FR("SPI heart beat thread is a client type VDIANA");
                        client_hb_type = MCTP_MESSAGE_TYPE_VDIANA;
                    }
                }
                else if (client_hb_type == MCTP_MESSAGE_TYPE_VDIANA) {
                    LOG_FR("Get a message from SPI heart beat thread");
                    if (manage_rx_vendor_packets(client_hb_fd, bytes_received) < 0)
                        break;
                }
                else {
...
void setJsonConfigName(char *const *argv, char *fileName)
{
    ...
    if (path != NULL) {
        ...
        sprintf((char *)(&fileJsonConfigName[parentLen]), "%s", argv[0]);
    }
    ...
    strncpy((char *)(&fileJsonConfigName[parentLen]), fileName, strlen(fileName)+1);
...
int main(int argc, char *const *argv)
{
	...
	pthread_mutex_init(&mctp_ctrl_mock_sync_mutex, &mctp_ctrl_mock_sync_mutex_attr);
    ...
    pthread_create(&fuzzerReceiverThread, nullptr, &runFuzzerReceiver, nullptr);
    ...
    if (!mctp_ctrl_fuzz_thread_is_ready) {
        pthread_cond_wait(&mctp_ctrl_mock_sync_cond, &mctp_ctrl_mock_sync_mutex);
    }
    pthread_mutex_unlock(&mctp_ctrl_mock_sync_mutex);
