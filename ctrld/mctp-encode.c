		...
		struct mctp_ctrl_cmd_prepare_ep_discovery *prep_ep_discovery)
{
	...
	encode_ctrl_cmd_header(&prep_ep_discovery->ctrl_msg_hdr, getRqDgramInst(),
...
bool mctp_decode_resp_set_eid(struct mctp_ctrl_resp_set_eid *set_eid)
{
	...
	/* Check EID and EID pool size exceed the thresholds */
	/* TBD */

	printf("VK: %s: eid_set: 0x%x, eid_pool_size: 0x%x\n", __func__,
					set_eid->eid_set, set_eid->eid_pool_size);
	return true;
}

/* Allocate Endpoint ID request */
bool mctp_encode_ctrl_cmd_alloc_eid(struct mctp_ctrl_cmd_alloc_eid *alloc_eid_cmd,
...
bool mctp_decode_resp_alloc_eid(struct mctp_ctrl_resp_alloc_eid *alloc_eid)
{
	...
	/* Sanity check for EID start and EID pool size */
	/* TBD */

	printf("VK: %s: eid_start: 0x%x, eid_pool_size: 0x%x\n", __func__,
...
bool mctp_decode_resp_get_routing_table(struct mctp_ctrl_resp_get_routing_table *routing_table)
{
	...
	printf("VK: %s: next_entry_handle: %d, number_of_entries: %d\n", __func__,
...
bool mctp_decode_resp_get_uuid(struct mctp_ctrl_resp_get_uuid *get_uuid_resp)
{
	...
	if (get_uuid_resp->uuid.raw)
		printf("VK: %s: sizeof uuid: %ld\n", __func__,
	...
	struct mctp_ctrl_cmd_discovery_notify *discovery_notify_cmd)
{
	...
	encode_ctrl_cmd_header(&discovery_notify_cmd->ctrl_msg_hdr,
			       getRqDgramInst(), MCTP_CTRL_CMD_DISCOVERY_NOTIFY);
	return true;
}
